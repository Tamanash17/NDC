...
function buildDistributionChain(chain?: DistributionChain): string {
  // Use chain from request first, then config defaults (no hardcoded fallback)
  const agencyId = chain?.links?.[0]?.orgId || config.distributionChain.orgCode;
  const agencyName = chain?.links?.[0]?.orgName || config.distributionChain.orgName || agencyId;

  // Use same structure as working OfferPrice builder
